...
We then show an implementation of this algorithm with time and space complexity $$poly(n)^{log n} T_D^n$$ in the QRAM model, and apply it to the Set Multicover problem.
...
While the time complexity of the best known classical algorithm is $$O(m(D+1)^n)$$, the time complexity of our quantum algorithm is $$poly(m,n)^{log n} T_D^n$$.
